...
RC tpcc_txn_man::run_payment(tpcc_query * query) {
    ...
    if (!query->by_last_name)
        ...
        customer =
                payment_getCustomerByLastName(query->w_id, query->d_id, query->c_last, &c_id);
    if (customer == NULL) {
        return finish(Abort);
    };
...
//    auto& arg = query->args.order_status;
    ...
    if (!query->by_last_name)
        customer =
                order_status_getCustomerByCustomerId(query->w_id, query->d_id, query->c_id);
    else
...
//    auto& arg = query;
    ...
    {
        o_id = 0;
        if (!delivery_getNewOrder_deleteNewOrder(d_id, query->w_id, reinterpret_cast<int64_t *>(o_id))) {
            // printf("oops0\n");
...
//    assert(idx_rc == RCOK);
    ...
    for (uint64_t i = 0; i < count; i++) {
        ...
        auto orderline = get_row((row_t *)orderline_shared->location, RD);

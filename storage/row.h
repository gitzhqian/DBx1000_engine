...
public:
    ...
    static const uint8_t LAT_CHASING_MASK = uint8_t{0x1} << 0;     //bits 2-1
    ...
    inline void mark_chasing(bool is_chasing)
    {
        if (is_chasing){
            version_t = version_t | LAT_CHASING_MASK;
        }else{
            version_t = version_t & (~LAT_CHASING_MASK);
        }
    }
    inline bool IsChasing() { return (version_t & LAT_CHASING_MASK) > 0; }
